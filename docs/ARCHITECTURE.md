...
工作区切换是前端编辑状态的安全边界。编辑器实例以 `workspaceId + sourceId/relativePath` 标识，切换时必须重建；编辑回调、草稿计时器、磁盘保存响应和异步文件读取都携带或捕获发起时的工作区身份，完成时再次比对当前身份，不匹配则丢弃。切换加载期间取消旧自动保存计时器并禁用输入；多个恢复请求乱序返回时只允许最后一次用户选择提交状态。选择资料若返回不同工作区，必须替换而非合并旧工作区的文件、Tab、脏状态、选区和上下文。

A2UI Surface 历史保存在 SQLite，工作区恢复时只恢复历史和活动引用，不自动占用中央视图；选择/恢复工作区、选择资料或打开文件都回到编辑器。交互成果的“关闭”只退出当前视图，不删除历史，用户仍可从“交互成果”重新查看。“永久删除”使用独立危险按钮和不可撤销确认，Rust 按工作区与 Surface ID 复核归属，并在单个事务中删除 Surface、同 Surface 协议消息和自动关联的 A2UI Result；Action 事件由外键级联删除。未通过的检查记录以可理解编号和状态显示，默认本地保留，也可经二次确认单条删除；Rust 同时约束工作区、检查记录 ID 和 `valid=0`，不连带删除 Surface、聊天、文件或其他记录。取消或事务失败时不改变任何记录，也不触碰工作区真实文件。

SQLite 启动先后执行完整性检查、单事务连续迁移和外键检查。Schema v8 为崩溃草稿保存正文 Hash；schema v9 新增独立 Result 聚合；schema v10 新增版本化内置模板和 Task/Result 绑定；schema v11 新增统一 Review Request/blocks 和 Patch/Result 应用引用。旧文件或 Surface 仍在显式打开时惰性关联，不批量复制正文。工作区恢复时列出全部草稿并恢复最新待处理审阅，区分待恢复、外部冲突、磁盘已写成功的陈旧记录和原文件不可用四种状态。目录文件与独立授权文件都先持久化草稿再写磁盘。
...
“我的成果”和左侧项目文件树是两个明确的导航域：托管 Result 不伪装成当前项目文件，也不在左侧树中泄露其物理路径。接受 `create_file` Review 后界面直接打开新 Result，并在标题区持续标明“保存在‘我的成果’”及提供返回成果列表的入口；Markdown 成果默认使用安全渲染的阅读视图，用户主动切换到编辑后才显示 Markdown 源文。选择左侧项目文件或重新点击主导航“工作台”会清除活动 Result，回到当前工作区编辑器，防止成果视图吞掉文件选择操作。
...
Patch V1 继续要求非空、唯一锚点。已授权的空白文本文件不会放宽 Patch 规则，而是进入 S2.5 的 `replace_empty_file` 完整内容 Review；Markdown 编辑器会忽略初始化阶段把真正零字节内容规范化成换行的回调，避免仅因打开文件就触发自动保存。Rust 将零字节、仅含 Unicode 空白或 UTF-8 BOM 的文本视为语义空白，但始终保存并复核原始字节 Hash；接受前零写入，应用时重新复核授权、可编辑状态、原始 Hash 和当前仍为空白，接受后使用现有 Patch/Revision 版本链，撤销精确恢复原始基线字节。

用户可以逐块取消或接受修改。应用命令不信任前端审阅状态，会基于当前磁盘内容再次执行全部校验，然后将语义块确定性转换为文本变更。写入失败或数据库版本记录失败时回滚本次已写文件。

AI 新建文本成果使用严格 `create_file` Review，只携带安全单层名称、Markdown/纯文本类型、完整候选内容和不透明工作区。接受前不建立文件、Result 或 Revision；接受后 Rust 使用 `create_new` 写入应用管理的“我的成果”，同事务建立 Result/初始 Revision 并记录 Review 应用状态。重复应用幂等，同名或外部变化进入冲突而不覆盖。读取 Result 时，Rust 从 SQLite 的 `output_result_id` 关系恢复可撤销的 Review ID 与原工作区 ID，并以 `ResultDocument.appliedReview` 返回，不依赖前端进程内“上次修改”指针。因此应用重启、离开成果页或从“我的成果”重新打开后，统一撤销入口仍可恢复。撤销必须把磁盘当前 Hash 与 AI 创建时的初始 Revision Hash 比较；后续手工保存或外部修改一律报冲突并保留成果。成功后回到“我的成果”并显示完成提示，失败时在当前成果页显示 Rust 返回的安全错误。待处理 Review 与已应用成果来源都以 SQLite 为事实源。

模型的 `document_patch`、`create_file` 和 `replace_empty_file` JSON 是机器协议，不是聊天正文。流式阶段前端把 JSON 信封显示为“正在生成可审阅方案”；Rust 校验成功后将聊天消息持久化为不含协议的用户可读摘要，结构化候选只保留在 Review 事实源；前端同时按 Review 类型和后端状态码渲染接受前零写入说明与审阅入口。这样重开会话也不会把有效 Review JSON 暴露为一串代码，普通 Markdown 回复仍走关闭原始 HTML 的安全预览。
