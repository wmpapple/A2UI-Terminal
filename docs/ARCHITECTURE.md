...
当前 Patch V1 仍要求非空、唯一锚点。已授权的零字节文本文件会由 Rust 在读取可信目标后明确报告为“暂不支持 AI 直接写入”，聊天层不为该已知能力限制再次请求 Provider，前端不显示手动重试，磁盘保持零写入。不得通过接受空锚点把该限制伪装成全文件替换；安全首次写入必须在 S2.5 统一 Review Pipeline 中复核零字节 Hash、授权、冲突、版本和撤销后实现。
