...
- PR CI 强制执行前端格式、lint、类型、单元/组件测试、关键模块覆盖率、Web Mock Playwright E2E，以及 Rust fmt/clippy/test 和桌面启动冒烟。
- 内部工作流生成未签名 NSIS/MSI 验收包；正式 `v*` 标签工作流使用受保护的 production environment、Windows 代码签名证书和 Tauri updater 私钥。
- 接入 Tauri updater/process 插件，启动后静默检查，设置页支持手动检查、下载、签名验证、安装和重启。
- 正式发布产出签名安装包、updater `.sig` 和 `latest.json`，默认先创建 Draft Release，人工验收后才发布给客户端。
- 增加只含版本、平台和记录数量的脱敏诊断导出；排除消息正文、文件内容、工作区路径和 Provider Key。
- 设置页提供“一键清除所有本地数据”，必须输入固定确认文本；数据库历史和系统凭据一并删除，真实项目文件保留。
- 更新 README、架构说明、开发指南、发布指南和隐私说明。

### 验收门槛

- 关键模块覆盖率四项指标均不低于 70%，Web Mock 三栏、A2UI 和 Patch/Diff/应用闭环 E2E 通过。
- 缺少签名证书、Publisher、updater 公钥或私钥时，正式发布工作流必须失败且不得创建可发布产物。
- 三处版本号和 tag 不一致时发布失败；安装包 Authenticode 或 updater 签名缺失时发布失败。
- updater 在未配置的内部构建中安全降级，不绕过签名验证，也不阻塞应用启动。
- 诊断文件和清理流程不包含或删除真实项目文件。
- 产品负责人完成内部安装包、正式签名材料和上一版本升级验证后确认阶段通过。
...
| 阶段 4：M1-A 工作区与编辑器          | 已通过 |            |
| 阶段 5：M1-B Provider 与上下文       | 已通过 |            |
| 阶段 6：M1-C Patch/Diff/撤销         | 已通过 |            |
| 阶段 7：A2UI Runtime 基础            | 已通过 |            |
| 阶段 8：M2 分发、更新与稳定性交付    | 已通过 | 2026-08-11 |

阶段 8 已于 2026-08-11 验收通过。当前既定实施计划已完成；正式发布仍须按发布指南配置签名材料并完成真实升级验收。
