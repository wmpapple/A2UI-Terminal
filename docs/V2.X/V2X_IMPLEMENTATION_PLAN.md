...
AI 接入是产品策略选择，可采用内置默认服务、仅 BYOK、默认服务 + BYOK；已有本地模型能力继续保留。M0 明确本次发布策略，只有选定内置服务的版本才将其服务端联调列为发布依赖。仅 BYOK 也可以形成完整 Product Beta，但不能宣称“无需 API 配置、开箱即用”。本轮没有替用户选定其中一条商业路径。
...
状态：待开始；客户端依赖 M1-A；M0 选择发布策略，仅在需要内置服务时推进服务端。客户端预计 4–6 人日；可选服务端另估。
