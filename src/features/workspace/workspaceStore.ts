...
export const createWorkspaceStore = (set: AppSet, get: AppGet): WorkspaceActions => {
  ...
  const beginWorkspaceTransition = () => ++workspaceTransition;
  const isCurrentWorkspaceTransition = (transition: number) => transition === workspaceTransition;
  ...
  return {
    ...
    selectWorkspace: async () => {
      ...
      await Promise.all(
        get().dirtyPaths.map((path) => get().persistDraft(path, sourceWorkspaceId))
      );
      ...
      try {
        ...
        if (!workspace) return;
        const [workspaceEntries, a2uiHistory, recoveryDraftSummaries, activeReviews] =
          await Promise.all([
            ...
            reviewController.listActive(workspace.id),
          ]);
        ...
        set({
          ...
          a2uiNotice: null,
          pendingDiff: activeReviews[0] ?? null,
          lastPatchApplication: null,
          lastReviewApplication: null,
          centerView: activeReviews.length > 0 ? 'diff' : 'editor',
        });
      } catch (error) {
        ...
        if (isCurrentWorkspaceTransition(transition)) set({ workspaceLoading: false });
      }
    },
    ...
    selectContextFiles: async () => {
      ...
      try {
        ...
        const switchesWorkspace = currentWorkspace?.id !== workspace.id;
        const recentWorkspaces = await workspaceController.listRecent();
        ...
        let activeReviews = switchesWorkspace ? [] : get().pendingDiff ? [get().pendingDiff!] : [];
        if (switchesWorkspace) {
          ...
          activeReviews = await reviewController.listActive(workspace.id);
        }
        ...
        set((state) => {
          ...
          return {
            ...
            activeSurfaceId: a2uiSurfaces[0]?.surfaceId ?? '',
            activeInspectionId: a2uiInspections[0]?.id ?? '',
            centerView: switchesWorkspace && activeReviews.length > 0 ? 'diff' : 'editor',
            files: [
              ...(switchesWorkspace
                ? []
            ...
            activePath: selectedPaths[0] ?? (switchesWorkspace ? '' : state.activePath),
            workspaceEntries: [
              ...(switchesWorkspace
                ? []
            ...
            saveStatusByPath: {
              ...(switchesWorkspace ? {} : state.saveStatusByPath),
            ...
            recoveryDraftSummaries,
            recoveryDrafts: !switchesWorkspace
              ? { ...state.recoveryDrafts, ...selectedRecoveryDrafts }
              : selectedRecoveryDrafts,
            dirtyPaths: switchesWorkspace
              ? []
              : state.dirtyPaths.filter((path) => !selectedPaths.includes(path)),
            selectedText: switchesWorkspace ? '' : state.selectedText,
            contextBySession: switchesWorkspace ? {} : state.contextBySession,
            contextReviewKeyBySession: switchesWorkspace ? {} : state.contextReviewKeyBySession,
            pendingDiff: switchesWorkspace ? (activeReviews[0] ?? null) : state.pendingDiff,
            lastPatchApplication: switchesWorkspace ? null : state.lastPatchApplication,
            lastReviewApplication: switchesWorkspace ? null : state.lastReviewApplication,
            patchBeforeByPath: switchesWorkspace ? {} : state.patchBeforeByPath,
            documentVersions: switchesWorkspace ? [] : state.documentVersions,
            versionPreview: switchesWorkspace ? null : state.versionPreview,
            versionHistoryPath: switchesWorkspace ? '' : state.versionHistoryPath,
          };
        });
      } catch (error) {
        if (isCurrentWorkspaceTransition(transition)) {
    ...
    acceptImportedSelection: async ({ workspace, documents }: SelectedWorkspaceFiles) => {
      ...
      if (switchesWorkspace) {
        await Promise.all(
          get().dirtyPaths.map((path) => get().persistDraft(path, currentWorkspace?.id))
        );
        if (!isCurrentWorkspaceTransition(transition)) return;
      }
      ...
      let a2uiInspections = get().a2uiInspections;
      let activeReviews = switchesWorkspace ? [] : get().pendingDiff ? [get().pendingDiff!] : [];
      if (!isWebMock && switchesWorkspace) {
        sessions = await chatController.listSessions(workspace.id);
        if (sessions.length === 0) {
          sessions = [
            await chatController.createSession(workspace.id, crypto.randomUUID(), '新对话'),
          ...
          contextBySession: Object.fromEntries(
            Object.entries(state.contextBySession).map(([sessionId, context]) => [
              sessionId,
              {
                ...
                documentSourceIds: (context.documentSourceIds ?? []).filter(
                  (id) => id !== sourceId
                ),
              },
            ])
    ...
    openFile: (path) => {
      ...
      return (async () => {
        try {
          ...
          if (!isCurrentWorkspaceTransition(transition) || get().workspace?.id !== workspaceId) {
            return;
          }
    ...
    updateFile: (path, content, expectedWorkspaceId) =>
      set((state) => {
        if (expectedWorkspaceId !== undefined && state.workspace?.id !== expectedWorkspaceId) {
          return state;
        }
        const file = state.files.find((item) => item.path === path);
        if (!file || file.editable === false || file.content === content) return state;
        return {
          files: state.files.map((item) => (item.path === path ? { ...item, content } : item)),
    ...
    persistDraft: async (path, expectedWorkspaceId) => {
      const state = get();
      ...
      if (
        ...
        !file?.contentHash ||
        file.editable === false ||
        (expectedWorkspaceId !== undefined && workspace?.id !== expectedWorkspaceId)
      ) {
        return;
      }
      if (!workspace) return;
      const workspaceId = workspace.id;
      try {
        await workspaceController.saveDraft(workspaceId, path, file.content, file.contentHash);
        set((current) =>
    ...
    saveFileToDisk: async (path, expectedWorkspaceId) => {
      const state = get();
      const workspace = state.workspace;
      const file = state.files.find((item) => item.path === path);
      ...
      try {
        ...
        if (!result) return;
        set((current) => {
          if (current.workspace?.id !== workspace?.id) return current;
        ...
        if (get().workspace?.id === workspace?.id && get().versionHistoryPath === path) {
